...
static server_options parse_args(int argc, char const **argv) {
  ...
  if (!retval.mcast_addr || !retval.shrd_fldr || !retval.max_space ||
      !retval.cmd_port || !retval.timeout || !retval.synchronized)
...
static bool sanitize_requested_path(std::string_view filename) {
  return (filename.size() > 0 && filename.find('/') == std::string_view::npos &&
          filename.find('\0') == std::string_view::npos && filename != "..");
}
...
static void index_files(int64 max_space) {
  ...
  for (auto &&entry : fs::directory_iterator(current_folder))
    if (fs::is_regular_file(entry.path())) {
      logger.trace("%s -> %ld", entry.path().c_str(),
...
static std::string make_filenames_list(std::string const &pattern) {
  ...
  for (auto &&entry : fs::directory_iterator(*so.shrd_fldr))
    if (fs::is_regular_file(entry.path())) {
      ...
      if (pattern.size() > 0 &&
          std::search(filename.begin(), filename.end(), pattern.begin(),
                      pattern.end()) == filename.end()) {
...
static bool try_alloc_file(fs::path file_path, ssize_t size) {
  ...
  if (current_space < size) {
    logger.trace("Refuse. File is too big!!!");
    return false;
  }

  if (fs::exists(file_path)) {
    logger.trace("Refuse. File exists!");
    return false;
  }
...
static bool try_read_file_stream(int msg_sock, fs::path file_path,
                                 size_t expected_size) {
  auto [succeess, reason] =
      recv_file_stream(msg_sock, file_path, expected_size, signal_fd);
  if (!succeess) {
...
static void sync_tell_other_to_delete(int sock, std::string filename) {
  ...
  auto del_packet =
      packet::make_simpl("DEL", packet_id, (uint8 const *)(filename.c_str()),
                         filename.size(), addr);
...
static void sync_sendfile_to_other(std::string addr_str, std::string port_str,
  ...
  setsockopt(stream_sock, SOL_SOCKET, SO_RCVTIMEO, (timeval *)&tv,
             sizeof(timeval));
  setsockopt(stream_sock, SOL_SOCKET, SO_SNDTIMEO, (timeval *)&tv,
             sizeof(timeval));
...
static void sync_save_files(int sock, in_addr mcast_addr) {
  ...
  std::sort(stored_files.begin(), stored_files.end(),
            [](auto x, auto y) { return x.file_size > y.file_size; });
  ...
  for (auto &&file : stored_files) {
    ...
    auto save_my_file_packet = packet::make_cmplx(
        "ADD", packet_id, file.file_size, (uint8 const *)(filename.c_str()),
        filename.size(), addr);
  ...
  for (;;) {
    auto time_left = chrono::duration_cast<chrono::milliseconds>(
        exit_time - chrono::steady_clock::now());
    if (time_left <= 0ms)
      break;
    ...
    if (pfd[0].revents & POLLIN) {
      ...
      if (cmdseq_to_filepath.count(response.cmd.get_cmd_seq()) &&
        ...
        workers.push_back(std::thread{sync_sendfile_to_other, addr_str,
                                      port_str, file_path, fields[0]});
      }
    }
  }
}
...
static void send_file(int sock, fs::path file_path) {
  ...
  if (msg_sock > 0) {
    ...
    logger.trace("Client didn't connect to download %s",
...
static void handle_request_hello(int sock, packet const &recv_packet) {
  ...
  if (!recv_packet.cmd.contains_required_fields(cmd_type::simpl,
                                                recv_packet.msg_len)) {
  ...
  packet send = packet::make_cmplx(
      "GOOD_DAY", recv_packet.cmd.get_cmd_seq(),
      av_space < 0 ? 0 : av_space, // Handle the case when no space.
      (uint8 const *)(so.mcast_addr->c_str()), so.mcast_addr->size(),
      recv_packet.addr);
...
static void handle_request_list(int sock, packet const &recv_packet) {
  ...
  std::string filenames =
      make_filenames_list((char const *)recv_packet.cmd.simpl.data);
  ...
  do {
    ...
    if (fnames_splited.empty() || fnames_splited.back().size() + 1 + entry_len >
                                      sizeof(command::simpl.data)) {
  ...
  for (auto &&fnames_chunk : fnames_splited) {
    packet send = packet::make_simpl("MY_LIST", recv_packet.cmd.get_cmd_seq(),
                                     (uint8 const *)(fnames_chunk.c_str()),
                                     fnames_chunk.size(), recv_packet.addr);
...
static void handle_request_get(int sock, packet const &recv_packet) {
  ...
  if (fs::exists(file_path)) {
    ...
    packet send =
        packet::make_cmplx("CONNECT_ME", recv_packet.cmd.get_cmd_seq(),
                           ntohs(port), (uint8 const *)filename_sv.data(),
                           filename_sv.size(), recv_packet.addr);
...
static void handle_request_add(int sock, packet const &recv_packet) {
  ...
  if (!recv_packet.cmd.contains_required_fields(cmd_type::cmplx,
                                                recv_packet.msg_len)) {
  ...
  if (is_space) {
    auto [socket, port] = init_stream_conn(chrono::seconds{*so.timeout});
    workers.push_back(std::thread{receive_file, socket, std::move(file_path),
                                  (size_t)file_size});

    packet send = packet::make_cmplx("CAN_ADD", recv_packet.cmd.get_cmd_seq(),
                                     ntohs(port), 0, 0, recv_packet.addr);

    logger.trace_packet("Responding to", send, cmd_type::cmplx);
    send_dgram(sock, send);
  } else {
    logger.trace("Could not add a file");
    packet send = packet::make_simpl("NO_WAY", recv_packet.cmd.get_cmd_seq(),
...
int main(int argc, char const **argv) {
  ...
  if (*so.synchronized) {
    for (auto &&entry : fs::directory_iterator(current_folder))
      if (fs::is_regular_file(entry.path())) {
        sync_tell_other_to_delete(sock,
