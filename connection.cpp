...
int accept_client_stream(int sock, chrono::seconds timeout) {
  ...
  timeval tv = chrono_to_posix(timeout);
  setsockopt(msg_sock, SOL_SOCKET, SO_RCVTIMEO, (timeval *)&tv,
             sizeof(timeval));
  setsockopt(msg_sock, SOL_SOCKET, SO_SNDTIMEO, (timeval *)&tv,
             sizeof(timeval));
  return msg_sock;
}
...
ssize_t send_stream(int sock, uint8 *buffer, size_t count, int signal_fd) {
  for (size_t i = 0; i < count;) {
    ...
    if (pfd[1].revents & POLLOUT) {
      ssize_t chunk_len =
          i + send_block_size > count ? count - i : send_block_size;
      ssize_t send_data = 0;
      send_data = send(sock, buffer + i, chunk_len,
                       MSG_NOSIGNAL); // Dont receive SIGPIPE
      if (send_data == -1) {
        return -1;
      }
...
recv_file_stream(int sock, fs::path out_file_path,
  ...
  if (expected_size && len_total != *expected_size) {
    logger.trace("Invalid size, got: %lu, expected: %lu", len_total,
                 *expected_size);
...
void send_dgram(int sock, packet const &packet) {
  ssize_t sent = sendto(sock, packet.cmd.bytes, packet.msg_len, 0,
                        (sockaddr const *)(&packet.addr), sizeof(packet.addr));
  if (sent != (ssize_t)packet.msg_len) {
    if (errno == EAGAIN)
...
packet recv_dgram(int sock) {
  packet retval{};
  ssize_t rcv_len = recvfrom(sock, retval.cmd.bytes, sizeof(retval.cmd.bytes),
                             0, (sockaddr *)&retval.addr, &retval.addr_len);
  retval.msg_len = rcv_len;

  if (rcv_len < 0) {
    if (errno == EAGAIN)
      logger.trace("Didn't received packet because of timeout.");
    else
      logger.syserr("sendto");
  }
