...
char const *command::get_head() const { return &(head[0]); }
...
uint64 command::get_cmd_seq() const { return be64toh(cmd_seq); }

void command::set_cmd_seq(uint64 val) { cmd_seq = htobe64(val); }
...
uint8 const *cmd_simpl_t::get_data() const { return (uint8 const *)data; }
...
uint8 const *cmd_cmplx_t::get_data() const { return (uint8 const *)data; }
...
uint64 cmd_cmplx_t::get_param() const { return be64toh(param); }

void cmd_cmplx_t::set_param(uint64 val) { param = htobe64(val); }
...
void command::clear() { bzero(&bytes[0], upd_max_data_size); }
...
packet::packet() : cmd{}, addr{} { addr_len = sizeof(addr); }

std::string_view packet::data_as_sv(cmd_type type) const {
  if (type == cmd_type::simpl) {
    return std::string_view{(char const *)cmd.simpl.get_data(),
                            msg_len - command::simpl_head_size};
  } else if (type == cmd_type::cmplx) {
    return std::string_view{(char const *)cmd.cmplx.get_data(),
                            msg_len - command::cmplx_head_size};
  } else
    return std::string_view{};
}

packet packet::make_simpl(char const *head, uint64 cmd_seq, uint8 const *data,
...
packet packet::make_cmplx(char const *head, uint64 cmd_seq, uint64 param,
