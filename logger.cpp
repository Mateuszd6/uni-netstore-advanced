...
void logger_t::trace_packet(char const *description, packet const &packet,
                            cmd_type type) {
  (void(description));
  (void(packet));
  (void(type));
  ...
  fprintf(stdout, "%s %s:%d: [%s] ", description,
          inet_ntoa(packet.addr.sin_addr), htons(packet.addr.sin_port),
          type == cmd_type::simpl ? "SIMPL" : "CMPLX");

  if (type == cmd_type::simpl) {
    fprintf(stdout, "{%.*s %lu \"%s\"}\n", 10, packet.cmd.head,
            packet.cmd.get_cmd_seq(), packet.cmd.simpl.get_data());
  } else {
    fprintf(stdout, "{%.*s %lu %lu \"%s\"}\n", 10, packet.cmd.head,
            packet.cmd.get_cmd_seq(), packet.cmd.cmplx.get_param(),
...
[[noreturn]] void logger_t::syserr(char const *fmt, ...) {
  std::lock_guard<std::mutex> m{logger_mutex};
...
[[noreturn]] void logger_t::fatal(char const *fmt, ...) {
  std::lock_guard<std::mutex> m{logger_mutex};
...
void logger_t::pckg_error(sockaddr_in const &addr, char const *reason) {
  ...
  fprintf(stderr, "[PCKG ERROR] Skipping invalid package from %s:%u. %s",
          sender_ip.c_str(), sender_port, reason ? reason : "");
  fprintf(stderr, "\n");
}
